...
function getPostcards($_db)
{
    ...
    while($row = $result->fetch_assoc())
    {
        ...
        . '</div><div class="body"><img src="' . $server_root . 'users/' . $row['IMAGE_NAME'] . '" width="300px"' . 'style=" -webkit-filter:' .$row['IMAGE_FILTER'] . '; filter:' . $row['IMAGE_FILTER'] . ';">' .  '      ' . $row['STATUS_TEXT'] . '</div></div>' ;
    }
